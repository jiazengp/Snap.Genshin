    ...
    public class DailySignInService
    {
        ...
        public async Task<List<Result>> SignInAsync()
        {
            ...
            Response<UserGameRoleInfo> rolesInfo =
                requester.Get<UserGameRoleInfo>($"{ApiTakumi}/binding/api/getUserGameRolesByCookie?game_biz=hk4e_cn");
            ...
            if (rolesInfo.ReturnCode == 0)
            {
                ...
                for (int i = 0; i < accountBindCount; i++)
                {
                    ...
                    Response<SignIn> signIn =
                        requester.Get<SignIn>($"{ApiTakumi}/event/bbs_sign_reward/info?act_id={ActId}&region={role.Region}&uid={role.GameUid}", true);
                    results.Add(signIn);
                    if (signIn.ReturnCode == 0)
                    {
                        ...
                        Response<SignInResult> result =
                            requester.Post<SignInResult>($"{ApiTakumi}/event/bbs_sign_reward/sign", data, true);
                        results.Add(result);
                    }
                }
            }
