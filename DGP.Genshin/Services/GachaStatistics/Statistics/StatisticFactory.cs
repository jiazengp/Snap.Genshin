    ...
    public static class StatisticFactory
    {
                ...
                foreach (GachaLogItem i in list)
                {
                    if (i.ItemType == itemType)
                    {
                        ...
                        if (!counter.ContainsKey(i.Name))
                        {
                            ...
                            if (itemType == "武器")
                            {
                                DataModel.Weapons.Weapon? weapon = MetadataViewModel.Instance.Weapons?.First(w => w.Name == i.Name);
                                counter[i.Name].Source = weapon?.Source;
                                counter[i.Name].Badge = weapon?.Type;
                            }
                            else if (itemType == "角色")
                            {
                                DataModel.Characters.Character? character = MetadataViewModel.Instance.Characters?.First(c => c.Name == i.Name);
        ...
        private static List<StatisticItem5Star> ListOutStatisticStar5(IEnumerable<GachaLogItem> items, int star5Count)
        {
            ...
            for (int i = 0; i < star5Count; i++)
            {
                ...
                SpecificBanner? matchedBanner = MetadataViewModel.Instance.SpecificBanners?.Find(b =>
                    //match type first
                ...
                counter.Add(new StatisticItem5Star()
                {
                    ...
                    Source = MetadataViewModel.Instance.FindSourceByName(currentStar5.Name),
        ...
        {
            //clone from metadata
            List<SpecificBanner>? clonedBanners = MetadataViewModel.Instance.SpecificBanners?.ClonePartially();
        ...
        {
            DataModel.Characters.Character? isc = MetadataViewModel.Instance.Characters?.FirstOrDefault(c => c.Name == item.Name);
            DataModel.Weapons.Weapon? isw = MetadataViewModel.Instance.Weapons?.FirstOrDefault(w => w.Name == item.Name);
            SpecificItem ni = new()
            {
                Time = item.Time
            };
