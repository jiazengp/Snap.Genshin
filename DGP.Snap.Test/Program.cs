...
namespace DGP.Snap.Test
{
    internal class Program
    {
        ...
        private static void CheckSalt(long time, string random,string result,List<string> salts,string saltbase)
        {
            for (int j = 0; j < template.Length; j++)
            {
                if (saltbase.Length < 32)
                {
                    ...
                    var computed = CreateDS(time, random, saltAfter);
        ...
        #region Encryption
        private static string CreateDS(long time,string random, string APISalt)
        {
            return ComputeMd5($"salt={APISalt}&t={time}&r={random}");
        }
